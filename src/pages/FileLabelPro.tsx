...
import stepSelectImg from "@/assets/images/fl-step-select.png";
import stepFormatImg from "@/assets/images/fl-step-format.png";
import stepPrintImg from "@/assets/images/fl-step-print.png";
import "./filelabelpro.css";

const steps = [
  {
    icon: stepSelectImg, name: "Select.",
    back: "Choose your files. Process individual folders or complete case file collections in a single workflow.",
  },
  {
    icon: stepFormatImg, name: "Generate.",
    back: "File LabelPro reads your file names and creates a formatted label set ready for review.",
  },
  {
    icon: stepPrintImg, name: "Print.",
    back: "Preview the final layout, verify alignment, and print directly to your preferred label sheets.",
  },
];

export default function FileLabelPro() {
    ...
    <div className="fl-page">
      ...
      <nav className="fl-nav">
        ...
        <ul className="fl-nav-links">
          <li><Link to="/file-labelpro" className="active">Home</Link></li>
      ...
      <section className="fl-hero">
        <h1>Generate <span className="fl-highlight">Print-Ready Labels</span><br />from Existing File Names.</h1>
        <p className="fl-hero-sub">Select your files, review the output, and print. No manual formatting.</p>
        <div className="fl-hero-actions">
          <Link to="/file-labelpro/pricing" className="fl-btn-primary">Try File LabelPro</Link>
          <Link to="/file-labelpro/how-it-works" className="fl-btn-outline">See How It Works</Link>
        </div>
      </section>

      {/* Video section */}
      <section className="fl-video-section">
        <h2>See File LabelPro in Action.</h2>
        <div className="fl-video-container">
          <video className="fl-home-video" src="/images/fl-hero.mp4" autoPlay loop muted playsInline />
        </div>
      </section>

      {/* Three steps */}
      <section className="fl-three-steps">
        <div className="fl-section-header">
          <h2>Three steps. That's it.</h2>
        </div>
        <div className="fl-steps-grid">
          {steps.map((s, i) => (
            <div key={s.name} className="fl-step-wrap">
              <div
              ...
              >
                <div className="fl-step-flip-inner">
                  <div className="fl-step-flip-front">
                    <div className="fl-step-card-cap">
                      <div className="fl-step-number">{i + 1}</div>
                    </div>
                    <span className="fl-step-card-icon">
                      <img src={s.icon} alt={s.name} />
                    </span>
                    <p className="fl-step-card-label">{s.name}</p>
                    <div className="fl-step-card-dots">
                      {[0, 1, 2].map(d => <span key={d} className={`fl-dot${d <= i ? " fl-dot-active" : ""}`} />)}
                    </div>
                  </div>
      ...
      <section className="fl-final-cta">
        <h2>From File Names to <span className="fl-highlight">Print-Ready Labels.</span></h2>
        <p>No manual formatting. No extra steps. No reprints.</p>
        <div className="fl-cta-actions">
          <Link to="/file-labelpro/pricing" className="fl-btn-primary">Launch File LabelPro</Link>
        </div>
      </section>
